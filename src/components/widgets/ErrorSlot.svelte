<script lang="ts">
  export let errorText = 'Sorry we can not find this page.';
  export let redirectPath = '/';
  export let redirectText = 'back to homepage';
</script>

<div class="mt-[70px] flex max-h-[500px] flex-col px-5 font-mono md:flex-row md:items-center">
  <div class="flex w-full max-w-[700px] flex-col gap-5">
    <p class="text-2xl font-light leading-normal md:text-2xl">{errorText}</p>
    <a
      href={redirectPath}
      target="_self"
      style="color:#fff"
      class="focus:shadow-outline-blue btn-primary inline max-w-[300px] rounded-lg border border-transparent px-4 py-3
			 font-medium leading-5
			shadow transition-colors duration-150 hover:bg-blue-700 focus:outline-none"
    >
      {redirectText}
    </a>
  </div>
